...
def execute():
    ...
    # rename columns
    col_remap = {'SCEDTimestamp': 'effective_from', 'SettlementPoint': 'settlement_point', 'LMP': 'lmp',
                 'RepeatedHourFlag': 'is_day_light_savings'}
    df.rename(columns=col_remap, inplace=True)

    # pivot dataframe so that each settlement_point is a column
    df = df\
        .pivot(index=['effective_from', 'is_day_light_savings'], columns='settlement_point', values='lmp')\
        .reset_index()
